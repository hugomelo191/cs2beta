...
export const getTeams = async (req, res, next) => {
    try {
        ...
        let whereConditions = [];
        if (search) {
            whereConditions.push(or(like(teams.name, `%${search}%`), like(teams.tag, `%${search}%`), like(teams.description, `%${search}%`)));
        }
        if (country) {
...
export const addTeamMember = async (req, res, next) => {
    try {
        const teamId = getParam(req, 'id');
        const { playerId, role } = getBody(req);
        if (!playerId || !teamId) {
...
export const removeTeamMember = async (req, res, next) => {
    try {
        ...
        await db.delete(teamMembers).where(and(eq(teamMembers.teamId, teamId), eq(teamMembers.playerId, playerId)));
...
// @access  Public
export const getFeaturedTeams = async (req, res, next) => {
    try {
...
// @access  Private
export const createTeamWithUser = async (req, res, next) => {
    try {
        // Validate input
        const validatedData = CreateTeamSchema.parse(getBody(req));
        const [newTeam] = await db.insert(teams).values({
...
// @access  Private
export const getUserTeamInvitations = async (req, res, next) => {
    try {
        ...
        const activeTeamMembers = await db.query.teamMembers.findMany({
            where: and(eq(teamMembers.teamId, userPlayer.teamId), eq(teamMembers.isActive, true)),
        });
        res.json({
            success: true,
            data: {
