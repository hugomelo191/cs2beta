...
export const getTournaments = async (req, res, next) => {
    try {
        ...
        let whereConditions = [];
        if (search) {
            whereConditions.push(or(like(tournaments.name, `%${search}%`), like(tournaments.description, `%${search}%`), like(tournaments.organizer, `%${search}%`)));
        }
        if (status) {
...
export const getUpcomingTournaments = async (req, res, next) => {
    try {
        ...
        const upcomingTournaments = await db.query.tournaments.findMany({
            where: and(eq(tournaments.status, 'upcoming'), gte(tournaments.startDate, new Date())),
            limit,
...
// @access  Public
export const getOngoingTournaments = async (req, res, next) => {
    try {
        const limit = getQueryInt(req, 'limit', 10);
...
export const registerTeamForTournament = async (req, res, next) => {
    try {
        ...
        const existingParticipant = await db.query.tournamentParticipants.findFirst({
            where: and(eq(tournamentParticipants.tournamentId, tournamentId), eq(tournamentParticipants.teamId, teamId)),
        });
...
export const updateParticipantStatus = async (req, res, next) => {
    try {
        ...
        const { status, seed, finalPosition } = getBody(req);
        // Update participant
...
// @access  Public
export const getTournamentsByStatus = async (req, res, next) => {
    try {
