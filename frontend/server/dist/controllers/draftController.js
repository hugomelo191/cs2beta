...
import { drafts, draftApplications } from '../db/schema.js';
...
// @access  Private
export const applyToDraft = async (req, res, next) => {
    try {
        ...
        const existingApplication = await db.query.draftApplications.findFirst({
            where: and(eq(draftApplications.draftId, draftId), eq(draftApplications.applicantId, currentUser.id)),
        });
...
// @access  Private
export const getDraftApplications = async (req, res, next) => {
    try {
        const { id: draftId } = req.params;
        const currentUser = req.user;
        // Check if draft exists
...
// @access  Public
export const getDraftsByStatus = async (req, res, next) => {
    try {
...
// @access  Public
export const getDraftsByCountry = async (req, res, next) => {
    try {
