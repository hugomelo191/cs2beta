...
export const gameController = {
    ...
    async getSteamPlayerStats(req, res) {
        try {
    ...
    async getFaceitPlayerStats(req, res) {
        try {
    ...
    async getLiveMatches(req, res) {
        try {
            ...
            const liveMatchesResponse = filteredMatches.map(item => ({
                ...
                current_score: item.match.current_score || { faction1: 0, faction2: 0 },
                map: item.match.map || 'TBD',
                started_at: item.match.started_at || Date.now(),
     ...
     */
    async getRegisteredTeamsHistory(req, res) {
        try {
            ...
            const historyResponse = filteredHistory.map(item => ({
                ...
                finished_at: item.match.finished_at,
                teams: {
                    registered_team: {
                        id: item.registered_team.id,
                        nickname: item.registered_team.name,
     ...
     */
    async getRegisteredTeams(req, res) {
        try {
     ...
     */
    async getRegisteredTeamStats(req, res) {
        try {
            const { teamId } = req.params;
            const registeredTeams = await registeredTeamsService.getRegisteredTeams();
            ...
            const teamMatches = teamHistory.filter(match => match.registered_team.id === teamId);
            // Calcular estatísticas básicas
            const stats = {
                total_matches: teamMatches.length,
                wins: teamMatches.filter(m => m.match.results?.winner === 'faction1' || m.match.results?.winner === 'faction2').length,
                losses: teamMatches.filter(m => m.match.status === 'FINISHED').length - teamMatches.filter(m => m.match.results?.winner).length,
                win_rate: 0,
                recent_form: teamMatches.slice(0, 5).map(m => m.match.results?.winner ? 'W' : 'L')
            };
     ...
     */
    async getPortugueseRankings(req, res) {
        try {
