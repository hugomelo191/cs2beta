...
export const getAllUsers = async (req, res, next) => {
    try {
        ...
        let whereConditions = [];
        if (search) {
            whereConditions.push(or(like(users.email, `%${search}%`), like(users.username, `%${search}%`), like(users.firstName, `%${search}%`), like(users.lastName, `%${search}%`)));
        }
        if (role) {
        ...
        const allUsers = await db.query.users.findMany({
            ...
            offset,
            limit,
            orderBy: sortOrder === 'desc' ? desc(users[sortColumn]) : asc(users[sortColumn]),
            columns: {
                id: true,
                email: true,
                username: true,
...
export const adminUpdateUser = async (req, res, next) => {
    try {
        const id = getParam(req, 'id');
        const { role, isVerified, ...profileData } = getBody(req);
