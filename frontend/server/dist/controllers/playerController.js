...
export const getPlayers = async (req, res, next) => {
    try {
        ...
        if (search) {
            whereConditions.push(or(like(players.nickname, `%${search}%`), like(players.realName, `%${search}%`)));
        }
        if (country) {
...
// @access  Public
export const getFeaturedPlayers = async (req, res, next) => {
    try {
        const limit = parseInt(req.query.limit) || 6;
        const featuredPlayers = await db.query.players.findMany({
            where: and(eq(players.isActive, true), sql `${players.stats}->>'views' IS NOT NULL`),
            with: {
                team: {
                    columns: {
                        id: true,
...
// @access  Public
export const getPlayersByPosition = async (req, res, next) => {
    try {
...
export const getPlayerLiveMatches = async (req, res, next) => {
    try {
        const { id } = req.params;
        // Buscar o jogador na base de dados
        const playersResult = await db
...
export const syncPlayerFaceit = async (req, res, next) => {
    try {
        const { id } = req.params;
        // Buscar o jogador na base de dados
        const playersResult = await db
