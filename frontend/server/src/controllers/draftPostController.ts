...
export const getDraftPosts = async (req: Request, res: Response) => {
  try {
    ...
    if (type) {
      conditions.push(eq(draftPosts.type, type as string));
    }
    
    if (role) {
      conditions.push(eq(draftPosts.role, role as string));
    }
    
    if (country) {
      conditions.push(eq(draftPosts.country, country as string));
    }
...
export const deleteDraftPost = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.id;
...
export const getDraftPostStats = async (req: Request, res: Response) => {
  try {
